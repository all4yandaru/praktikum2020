...
glimpse(un_smp) # mirip str dari dplyr
```
...
```{r}
un_smp <- un_smp %>% 
  mutate(tahun = as.character(tahun)) # ngubah tipe data tahun
glimpse(un_smp)
```
...
```{r}
set.seed(270719)
un_smp_split <- initial_split(un_smp) # untuk membagi data training dan testing
un_smp_split
```
...
# root-mean-square error ( RMSE ) makin kecil makin bagus, gamungkin 0
# koefisien determinasi ( Rsq ) makin besar dapat menjelaskan keseluruhan variasi data / data semakin jelas dan pasti
# mean absolute error ( MAE ) adalah ukuran umum kesalahan / akurasi
```
